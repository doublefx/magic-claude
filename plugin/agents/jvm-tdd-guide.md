...
model: sonnet
skills: tdd-workflow, backend-patterns, magic-claude-mem:claude-mem-context, serena-code-navigation
hooks:
  Stop:
    - hooks:
        - type: prompt
...
5. **No leftovers** — No TODOs, no stubs, no commented-out code, no debug statements (System.out.println, e.printStackTrace).
